# Taken from https://stackoverflow.com/a/17364637 with many thanks
# Accepts a version string and prints it incremented by one.
# Usage: increment_version <version> [<position>] [<leftmost>]
increment_version_rc() {
	base=${1%rc[0-9]*}
	export rc_version=$( echo $1 |sed -e "s/^v[0-9.]*rc//"  )	
	rc_version_new=$( increment_version_normal $rc_version )
	echo "${base}rc${rc_version_new}"
	return 0
}
...
increment_version(){
	if [[ ! -z $( echo $1 | grep rc ) ]]; 
	then 
		increment_version_rc $1;
	else
		increment_version_normal $1;
	fi
}


increment_version_normal() {
